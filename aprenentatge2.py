def aprenentatge(image,mapren,db):
        ...
        for row in cursor.fetchall(): # recorro tots els tags de la imatge
                if mapren.has_key(key): # si la classe ja ha estat declara al dict
                        tmp=mapren[key] # obtinc el dict que hi ha dins del dict, aquest conte tag i repeticions 
                        if tmp.has_key(row[0]): # si el tag ja existeix li sumo un 
                                tmp[row[0]]+=1
                        else: # si no existeix, el creo 
                                tmp[row[0]]=1
                        mapren[key]=tmp
                else: # si la clase no ha estat declarada
                        tmp[row[0]]=1; # creo el dict que hi ha dins de mapren amb el tag normalitzat
